...
def connect_ftp(
    ...
    for file_info in l_file:
        print(get_file_name(file_info))
    for dir_info in l_dir:
        l_subdir = list_folder(connection, get_file_name(dir_info))
        print(l_subdir)
        save_file(connection, f"{actual_path}/{get_file_name(file_info)}", f"{save_path}/{get_file_name(file_info)}")
...
if __name__ == "__main__":
    connect_ftp(host="ftp.heanet.ie", save_path="C:/Users/pegar/Onedrive/Desktop/LPC/practica7/practica7")